...
=   eDNA and Traditional Data merge
...
-   This section prepares data to identify common and exclusive species between different monitoring methods and sampling sites. The resulting tables can be used to create Venn diagrams.
    ...
    spp_comm <-
    merged_methods_longer %>% 
      filter(Taxon %in% species_level_edna) %>%
      group_by(Taxon) %>%
      summarise(n_Level = n_distinct(Method)) %>%
      filter(n_Level >= 2) %>%
      select(Taxon)
