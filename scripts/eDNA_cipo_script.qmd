...
-   Traditional methods capture data.

    ```{r}
    traditional_results <-  
      read.csv(paste0(data_path, "/coleta_tradicional-eDNA_cipo-151125.csv"),
               sep = ",", 
               check.names = FALSE) %>%
      mutate(across(everything(), ~ ifelse(. == "", NA, .))) %>% 
      as_tibble()
...
```{r}
    ...
    taxa_site_wider <-
      ...
      mutate("%" = round(Reads / `Sample abundance` * 100, 2)) %>% 
      reframe("Local" = `Metadata 1`,
              "Taxon (Curated ID)" = `Curated ID`,
              "Abundância" = Reads,
              `%`) %>% 
      unique() %>% 
      pivot_wider(id_cols = `Taxon (Curated ID)`,
                 names_from = Local,
                 values_from = `%`)
...
-   Assign colors for each reservoir site and for all sites.
    ...
    site_colors <- c("SC1" = "#67322EFF", "SC2" = "#9C593EFF", "SC3" = "#DDA569FF", "SC4" = "#3F4330FF",
                     "SC5" = "#2C6B67FF", "SC6" = "#2A4866FF", "SC7" = "#6592B0FF")
...
-   Create palletes
    ...
    ```{r}
    colors_base <- rev(c("#67322EFF","#9C593EFF","#DDA569FF","#3F4330FF","#2C6B67FF","#2A4866FF","#6592B0FF"))
...
-   Species comparation per site
    ...
    merged_methods_longer_pa %>%
      ...
      facet_grid(cols = vars(Site),
                 ...
                 scales = "free",
                 drop = TRUE) +
      # scale_fill_manual(values = c("Absent" = "#f8766c",
      #                              "Present" = "#008a0e")) +
      theme(panel.grid.major = element_line(color = "grey",
                                            linewidth = 0.2,
                                            linetype = 1),
