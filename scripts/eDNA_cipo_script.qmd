...
-   **Load custom functions.** Custom functions for taxonomy retrievel.
    ...
    # Update Characidae taxonomy
    source("/home/gabriel/functions/update_characidae_taxonomy.R")
    ```
...
-   **Update Characidae Taxonomy** Use the `update_characidae_taxonomy` function to append the correct Family and Subfamily based on the reclassification by Melo et al. (2024) of taxa previously belonging to Characidae Family 

    ```{r}
    raw_curated_results <-
      raw_curated_results %>% 
      update_characidae_taxonomy(genus_col = "Genus (Curated ID)", 
                                 species_col = "Curated ID")
    ```
...
-   **Calculate taxonomic metrics.** Count the total number of unique Orders, Families, Genera, and Species identified in the final dataset.
    ...
    site_stats <-
      ...
      reframe("Local" = `Metadata 1`,
              ...
              "Familias" = n_distinct(`Family (Updated)`),
...
-   **Matrix creation.** Presence/absence transformation and computation of overall species accumulation curve.
    ...
    accum_sc1 <- 
      specaccum(sc1_taxa_wider_jac[3:ncol(sample_taxa_wider_jac)],
                method = "random",
                permutations = 999)
    ...
    accum_sc2 <- 
      specaccum(sc2_taxa_wider_jac[3:ncol(sample_taxa_wider_jac)],
                method = "random",
                permutations = 999)
    ...
    accum_sc3 <- 
      specaccum(sc3_taxa_wider_jac[3:ncol(sample_taxa_wider_jac)],
                method = "random",
                permutations = 999)
    ...
    accum_sc4 <- 
      specaccum(sc4_taxa_wider_jac[3:ncol(sample_taxa_wider_jac)],
                method = "random",
                permutations = 999)
    ...
    accum_sc5 <- 
      specaccum(sc5_taxa_wider_jac[3:ncol(sample_taxa_wider_jac)],
            method = "random",
                permutations = 999)
    ...
    accum_sc6 <- 
      specaccum(sc6_taxa_wider_jac[3:ncol(sample_taxa_wider_jac)],
            method = "random",
                permutations = 999)
    ...
    accum_sc7 <- 
      specaccum(sc7_taxa_wider_jac[3:ncol(sample_taxa_wider_jac)],
                method = "random",
                permutations = 999)
...
-   **Calculate diversity indices.** Compute Shannon, Simpson, and Richness (Observed) metrics for each site/method.
    ...
    alpha_sample_tbl <- 
      ...
      reframe(Amostra,
              Local, 
              Observed = specnumber(c_across(3:ncol(.))),
             Shannon = diversity(c_across(3:ncol(.)), index = "shannon"),
             Simpson = diversity(c_across(3:ncol(.)), index = "simpson")) %>%
      ungroup() %>%
    ...
    alpha_site_tbl <- 
      ...
      rowwise() %>%
      reframe(Site,
              Observed = specnumber(c_across(2:ncol(.))),
              Shannon = diversity(c_across(2:ncol(.)), index = "shannon"),
              Simpson = diversity(c_across(2:ncol(.)), index = "simpson")) %>%
      ungroup() %>%
      pivot_longer(cols = c(Observed, 
                            Shannon, 
                            Simpson),
