...
-   **Taxonomy correction**
    ...
    ```{r}
    upd_eDNA_traditional_methods_list <-
      read.csv(paste0(data_path, "/eDNA_traditional_methods_list-2026-05-12_corrigido.csv"), 
               sep = ",",
               check.names = FALSE) %>%
      select(-c(1)) %>% 
      as_tibble()
    
    anti_join(upd_eDNA_traditional_methods_list, eDNA_traditional_methods_list)
    anti_join(eDNA_traditional_methods_list, upd_eDNA_traditional_methods_list)
    ```
...
# Convert the long table to a wide format (IDs & Samples x Sites)
taxa_sample_wider_alluvial <- 
  taxa_sample_longer %>%
  select(c(Site, 
           Sample,
           Taxon,
           `%`)) %>%
  pivot_wider(id_cols = c(Taxon,
                          Sample),
              names_from = Site,
              values_from = `%`,
              values_fn = sum_uniq,
...
taxa_sample_wider_alluvial %>%
  mutate(Taxon = factor(Taxon,
                        levels = rev(spp_levels))) %>%  
  ggplot(aes(y = Taxon,
             axis1 = SC1, axis2 = SC2, axis3 = SC3, axis4 = SC4, axis5 = SC5, axis6 = SC6, axis7 = SC7)) +
  geom_alluvium()
  facet_grid(cols = vars(Site),
             rows = vars(Family),
             space = "free",
