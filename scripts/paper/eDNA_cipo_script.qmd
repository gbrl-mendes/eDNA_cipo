      ...
      mutate("Final ID (BLASTn)" =
             ...
             "Final ID (BLASTn)" =
               ifelse(`Final ID (BLASTn)` %in% ("Rhamdia aff."),
                      "Rhamdia aff. quelen", `Final ID (BLASTn)`),
...
-   **Taxonomy corrections.** Change specific assignments based on recent taxonomic updates.

    ```{r}
    raw_curated_results <-
      raw_curated_results %>%
      mutate("Curated ID" =
               ifelse(`Curated ID` %in% ("Megaleporinus obtusidens"), 
                      "Arhinolemur obtusidens", `Curated ID`),
             "Genus (Curated ID)" =
               ifelse(`Genus (Curated ID)` %in% ("Megaleporinus"),
                      "Arhinolemur", `Genus (Curated ID)`),
             "Family (Curated ID)" =
               ifelse(`Genus (Curated ID)` %in% ("Rhamdia"),
                      "Heptapteridae", `Family (Curated ID)`))
    ```
...
-   **Recalculate relative abundances.** Compute new relative abundance percentages (RRA) for each ASV based on the final filtered read counts per sample and site.
    ...
    curated_results <-
          ...
          mutate("Sample total abundance" = sum(`ASV absolute abundance`),
                 "Relative abundance on sample" = `ASV absolute abundance`/ 
                   `Sample total abundance` * 100) %>%
          ...
          mutate("Site total abundance" = sum(`ASV site abundance`),
                 "Relative abundance on site" = `ASV site abundance`/ 
                   `Site total abundance` * 100) %>%
...
-   **Review taxonomy.** Ensure species names in the traditional dataset match the nomenclature used in the eDNA dataset to allow direct comparison.
    ...
    # Retrieve taxIDs
    # taxids_trad <- retrieve_taxid(traditional_results$Genus)

    # Retrieve complete taxonomy
    curated_taxonomy_trad <- retrieve_taxonomy(taxids_trad)

    # Select only necessary information
    curated_taxonomy_trad_rdx <-
      curated_taxonomy_trad %>%
      filter(class == "Actinopteri") %>%
      select(order, family, genus) %>%
      reframe("Order" = order,
              "Family" = family,
    ...
    traditional_results <-
      traditional_results %>%
      left_join(curated_taxonomy_trad_rdx,
                by = join_by(Genus)) %>%
...
-   **Taxonomy corrections.** Change specific assignments based on recent taxonomic updates.
    ...
    traditional_results <-
      traditional_results %>%
      mutate("Family" = ifelse(`Genus` %in% ("Rhamdia"),
                      "Heptapteridae", `Family`),
             "Family" = ifelse(`Genus` %in% ("Knodus"),
                      "Acestrorhamphidae", `Family`))
    ```
...
-   **Export full species list of merged methods**

    ```{r}
    eDNA_traditional_methods_list <-
    merged_methods_longer %>% 
      select(-c(Site, Ocurrence)) %>% 
      unique() %>% 
      print()

    # Export table
    write.csv(eDNA_traditional_methods_list,
              paste0(tbl_path, "/eDNA_traditional_methods_list", "-",
                     Sys.Date(), ".csv", sep = ""))
    ```
...
```{r}
# Recalculate the percentages for Species and Site levels
taxa_site_longer_alluvial <- 
  taxa_site_longer %>% 
  group_by(Site, 
       Taxon) %>%
  summarise(reads_Taxon = sum(Reads), 
        .groups = "drop") %>%
  group_by(Site) %>%
  mutate(total_site = sum(reads_Taxon),
     perc = reads_Taxon / total_site * 100) %>%
  ungroup() %>%
  print()
...
taxa_site_longer_alluvial %>%
  ...
  theme(panel.grid.major = element_line(color = "grey",
        ...
        axis.title = element_text(size = 16,
                                  face = "bold")) +
  labs(title = "Taxa evolution between sites",
       fill = "%",
       x = "Site",
       y = "Taxon")
...
# Recalculate the percentages for Family and Site levels
taxa_site_longer %>% 
  group_by(Site,
           Family) %>%
  summarise(reads_Family = sum(Reads),
            .groups = "drop") %>%
  group_by(Site) %>%
  mutate(total_site = sum(reads_Family),
         perc = reads_Family / total_site * 100) %>%
  ungroup() %>%
...
```{r}
# Plot Species level only for Order Siluriformes
taxa_site_longer %>% 
  filter(Order == "Siluriformes") %>%
  group_by(Site,
           Taxon) %>%
  summarise(reads_Taxon = sum(Reads), 
        .groups = "drop") %>%
  ...
  theme(panel.grid.major = element_line(color = "grey",
        ...
        axis.title = element_text(size = 16,
                                  face = "bold"),
        legend.position = "none") +
  labs(title = "Taxa changes between sites (Siluriformes)",
       # fill = "%",
       x = "Site",
       y = "Reads abundance")
